//
//  ExploreSelectedIntrestedScreen.swift
//  Source-App
//
//  Created by iroid on 18/04/21.
//
...
class ExploreSelectedIntrestedScreen: UIViewController {
    ...
    func initialDetail(){
        ...
        self.getPostAPI(page: self.page,postType: self.postType,search: self.searhText,userName: self.searchUserName, exploreId: self.exploreId)
...
//            self.searhText = nil
    ...
    func deletePostAPI(postObj: PostReponse){
        ...
        if Utility.isInternetAvailable(){
            ...
            PostService.shared.deletePost(id: id) { [weak self] (statusCode, response) in
                ...
                strongSelf.page = 1
                strongSelf.getPostAPI(page: strongSelf.page, postType: strongSelf.postType, search: strongSelf.searhText, userName: strongSelf.searchUserName, exploreId: strongSelf.exploreId)
    ...
    //MARK: - POST API
    func getPostAPI(page: Int,postType: String?,search: String?,userName: String?,exploreId: Int?){
//        self.view.endEditing(true)
        if Utility.isInternetAvailable(){
            if !self.refreshControl.isRefreshing{
//                Utility.showIndicator()
            }
            let data = GetPostRequest(post_type: postType, search: search,username: userName, explore: exploreId ?? 0, groupId: nil)
            ...
            if self.hasMorePage{
                if let metaTotal = self.meta?.total{
                    if self.itemArray.count != metaTotal{
                        ...
                        self.page += 1
                        self.getPostAPI(page: self.page,postType: self.postType,search: self.searhText,userName: self.searchUserName, exploreId: exploreId)
                    }
                }
            }
        }
...
extension ExploreSelectedIntrestedScreen: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
...
extension ExploreSelectedIntrestedScreen: UITableViewDelegate, UITableViewDataSource{
...
//                cell.readMoreButton.isHidden = true
            ...
            cell.onShare = { [weak self] in
                ...
                vc.onSendMessage = { [weak self] in
                    ...
                    vc.postItemDataDetailData = self?.itemArray[indexPath.row]
                    vc.messageNavigationType = MessageNavigationType.exploreScreen.rawValue
                    self?.navigationController?.pushViewController(vc, animated: true)
...
extension ExploreSelectedIntrestedScreen: ExpandableLabelDelegate{
...
extension ExploreSelectedIntrestedScreen: UITextFieldDelegate{
...
extension ExploreSelectedIntrestedScreen: EnterNewPostDelegate{
    func uploadNewPost() {
        self.page = 1
        self.postType = self.getPostType()
        self.getPostAPI(page: self.page, postType: self.postType, search: self.searhText,userName: self.searchUserName, exploreId: self.exploreId)
...
//MARK:- CACHE IMAGE{
extension ExploreSelectedIntrestedScreen{
